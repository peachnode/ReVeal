...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default='chrome_debian/balanced')  # accepts any dataset
    ...
    # Always use output/<dataset>/ — find 'output' relative to this file
    here = Path(__file__).resolve().parent
    out_root = next((p / 'output' for p in [here, *here.parents] if (p / 'output').is_dir()), Path('output'))
    ds = str((out_root / dataset).as_posix() + '/')
